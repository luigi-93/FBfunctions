...
async function IoCSetup(iocContainer, options = {
    ...
    try {
        iocContainer
            .bind(utilityKeys_1.registry.FirebaseAdmin)
            .toDynamicValue(() => (0, setAuth_1.initializeFirebaseAdmin)(needAdminPrivileges))
            .inSingletonScope();
        iocContainer
            .bind(utilityKeys_1.registry.FirebaseJwtAuthStrategy)
            .toDynamicValue((context) => {
            try {
                const firebaseAdmin = context.container.get(utilityKeys_1.registry.FirebaseAdmin);
                const strategyLogger = context.container.get(utilityKeys_1.SYMBOLS.CUSTOM_LOGGER);
    ...
    if (!iocContainer.isBound(utilityKeys_1.SYMBOLS.API_KEY_VALIDATOR)) {
        iocContainer
            .bind(utilityKeys_1.SYMBOLS.API_KEY_VALIDATOR)
            .to(validationApiKey_1.ApiKeyValidator)
            .inSingletonScope();
    }
    if (!iocContainer.isBound(utilityKeys_1.SYMBOLS.STORAGE_ADAPTER)) {
        iocContainer
    ...
    try {
        ...
        if (!iocContainer.isBound(utilityKeys_1.SYMBOLS.API_KEY_MANAGER)) {
            iocContainer
                .bind(utilityKeys_1.SYMBOLS.API_KEY_MANAGER)
                .to(apiKeyManager_1.ApiKeyManager)
                .inSingletonScope();
        }
        const manager = iocContainer.get(utilityKeys_1.SYMBOLS.API_KEY_MANAGER);
        const generatedKeys = [];
        await Promise.all(apiKeys.map(async (keyConfig) => {
            const keyResult = await manager.create(keyConfig.name, {
                scopes: keyConfig.scopes || [utilityKeys_1.SecurityScopes.User],
        ...
        iocContainer
            .bind(utilityKeys_1.registry.FirebaseApiKeyAuthStrategy)
            .toDynamicValue((context) => {
            try {
                const strategyLogger = context.container.get(utilityKeys_1.SYMBOLS.CUSTOM_LOGGER);
        ...
        try {
            ...
            iocContainer
                .bind(utilityKeys_1.SYMBOLS.AUTH_STRATEGY_FACTORY)
